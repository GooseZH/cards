...
public class JedisFactory {
	...
	private static ShardedJedis getInstance() {
			...
			if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)) {
				try {
					...
					if (shardedJedisPool == null) {
						...
						logger.info(">>>>>>>>>>> xxl-sso, JedisFactory.ShardedJedisPool init success.");
					}
		...
		if (shardedJedisPool == null)
			throw new NullPointerException(">>>>>>>>>>> xxl-sso, JedisFactory.ShardedJedisPool is null.");
	...
	public static Object getObjectValue(String key) {
		...
		try {
			...
			if (bytes != null && bytes.length > 0) obj = unserialize(bytes);
		} catch (Exception e) {
			logger.info("", e);
		} finally {
			client.close();
